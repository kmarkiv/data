...
def update_sql(data, id):
    print data, id
    SQL = "UPDATE hpi_country SET hpi_cum=%s WHERE id=%s "
    cur.execute(
        SQL,
        (data, id))
...
for row in data:
    ...
    sql = "SELECT * FROM hpi_country WHERE country='%s' ORDER BY year ASC" % row['country']
    countries = get_mysql_data(sql)
    hpi_sum_data = {}
    for age in countries:
        hpi_sum = hpi_sum_data.get(age['occupation'], 0)
        hpi_sum += age['hpi_sum']
        hpi_sum_data[age['occupation']] = hpi_sum
        k += 1
